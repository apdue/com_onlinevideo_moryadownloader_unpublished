...
import com.video.victusadownloaders.WatchVideo.download.ui.DownloadDialog;
...
import com.video.victusadownloaders.WatchVideo.player.event.PlayerServiceExtendedEventListener;
import com.video.victusadownloaders.WatchVideo.player.helper.PlayerHelper;
import com.video.victusadownloaders.WatchVideo.player.helper.PlayerHolder;
...
//        }
    ...
    public void onDestroy() {
        ...
        if (activity.isFinishing() && player != null && player.videoPlayerSelected()) {
            PlayerHolder.stopService(AppNew.getAppContext());
        } else {
...
//        if (new PrefManagerVideo(getActivity()).getBoolean(SplashActivity.TAG_YOUTUBE_DOWNLOAD_ENABLED)){
//            Log.d("TAGGEGE", "initViews: hiding");
...
//        }
        ...
        if (prf.getBoolean(SplashActivityScr.TAG_DOWNLOAD_ENABLED)) {
    ...
    protected void initListeners() {
        ...
        if (!PlayerHolder.bound) {
            ...
            PlayerHolder.startService(AppNew.getAppContext(), false, this);
        }
    }

    private void initThumbnailViews(@NonNull StreamInfo info) {
        if (!TextUtils.isEmpty(info.getThumbnailUrl())) {
            GlideUtils.loadThumbnail(AppNew.getAppContext(), thumbnailImageView, info.getThumbnailUrl());
        }

        if (!TextUtils.isEmpty(info.getUploaderAvatarUrl())) {
            GlideUtils.loadAvatar(AppNew.getAppContext(), uploaderThumb, info.getUploaderAvatarUrl().replace("s48", "s720"));
        }
    }
    ...
    private void openMainPlayer() {
        if (playerService == null) {
            PlayerHolder.startService(AppNew.getAppContext(), true, this);
            return;
        }
        if (currentInfo == null) {
            return;
        ...
        final Intent playerIntent = NavigationHelper.getPlayerIntent(activity, MainPlayer.class, queue, null, true);
            ...
            public void onReceive(final Context context, final Intent intent) {
                ...
                switch (intent.getAction()) {
                    ...
                    case ACTION_PLAYER_STARTED:
                        ...
                        if (!PlayerHolder.bound) {
                            PlayerHolder.startService(AppNew.getAppContext(), false, VideoDetailFragment.this);
                        }
                        break;
                }
            }
    ...
    private void updateOverlayData(@Nullable final String title, @Nullable final String uploader, @Nullable final String thumbnailUrl) {
        ...
        if (!TextUtils.isEmpty(thumbnailUrl)) {
            GlideUtils.loadThumbnail(AppNew.getAppContext(), overlayThumbnailImageView, thumbnailUrl);
        }
    }
