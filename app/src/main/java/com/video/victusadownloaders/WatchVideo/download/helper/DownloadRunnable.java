package com.video.victusadownloaders.WatchVideo.download.helper;
...
public class DownloadRunnable extends Thread {
	...
	private void releaseBlock(DownloadMission.Block block, long remain) {
	...
	public void run() {
		boolean retry = false;
		DownloadMission.Block block = null;
		int retryCount = 0;
		SharpStream f;
		...
		while (mMission.running && mMission.errCode == DownloadMission.ERROR_NOTHING) {
			...
			try {
				...
				if (e instanceof DownloadMission.HttpError && ((DownloadMission.HttpError) e).statusCode == DownloadMission.ERROR_HTTP_FORBIDDEN) {
